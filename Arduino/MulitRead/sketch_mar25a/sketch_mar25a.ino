...
void setup() {
  ...
  String json = "{\"status\":201, \"msg\":\"Start Arduino\", \"nfc\":\"\", \"reader\":\"" + command + "\"}";
  Serial.println(json);
...
void initPN532(Adafruit_PN532& nfc, String readerName) {
  ...
  if (!versiondata) {
    String json = "{\"status\":400, \"msg\":\"NFC Connect Fail\", \"nfc\":\"\", \"reader\":\"" + readerName + "\"}";
    Serial.println(json);
    // Serial.println("❌ " + readerName + " 未偵測到！");
  } else {
    // Serial.print("✅ " + readerName + " 已連接！ Firmware: ");
    String json = "{\"status\":202, \"msg\":\"Success Connect\", \"nfc\":\"\", \"reader\":\" Firmware:" + readerName + " " + (versiondata, HEX) + "\"}";
    Serial.println(json);
